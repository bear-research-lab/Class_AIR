...
def update_plot(*args):
    ...
    R_roll = Rx(roll)
    R_roll_pitch = Ry(pitch) @ Rx(roll)
    R_rpy = Rz(yaw) @ Ry(pitch) @ Rx(roll)

    # Draw four frames at different origins so they are easy to compare
    draw_frame(ax, R0,            origin=[0.0, 0.0, 0.0], label='Original')
    draw_frame(ax, R_roll,        origin=[2.0, 0.0, 0.0], label='Roll only')
    draw_frame(ax, R_roll_pitch,  origin=[4.0, 0.0, 0.0], label='Roll+Pitch')
    draw_frame(ax, R_rpy,         origin=[
               6.0, 0.0, 0.0], label='Roll+Pitch+Yaw')
...
# ============================================================
root = tk.Tk()
root.title("3D RPY Coordinate Frame Visualizer")
...
# Roll slider
ttk.Label(control_frame, text="Roll (deg)").grid(row=0, column=0, sticky="w")
...
roll_slider.grid(row=0, column=1, padx=10, pady=5)
...
# Yaw slider
ttk.Label(control_frame, text="Yaw (deg)").grid(row=2, column=0, sticky="w")
yaw_slider = tk.Scale(
    control_frame, from_=-180, to=180, orient=tk.HORIZONTAL,
    resolution=1, variable=yaw_var, command=update_plot, length=350
)
yaw_slider.grid(row=2, column=1, padx=10, pady=5)
